...
class Sigil(db.Model):
    ...
    def to_dict(self):
        return {
            ...
            'ethical_profile': self.ethical_profile if self.ethical_profile is not None else [],
            'capabilities': self.capabilities if self.capabilities is not None else [],
            'user_attributes': self.user_attributes if self.user_attributes is not None else {},
            'proofs': self.proofs if self.proofs is not None else {},
            'linked_wallet_address': self.linked_wallet_address,
            'sigil_fingerprint': self.sigil_fingerprint,
            'verifiable_credentials': self.verifiable_credentials if self.verifiable_credentials is not None else []
        }
...
class Lock(db.Model):
    ...
    def to_dict(self):
        return {
            ...
            'access_policy': self.access_policy if self.access_policy is not None else {},
            'access_message': self.access_message,
            'on_chain_actions': self.on_chain_actions if self.on_chain_actions is not None else []
        }
...
class AccessLog(db.Model):
    ...
    def to_dict(self):
        return {
            ...
            'transaction_receipts': self.transaction_receipts if self.transaction_receipts is not None else [],
