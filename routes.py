...
def create_sigil():
    try:
        ...
        db_sigil = Sigil.from_metadata(sigil_metadata.to_dict(), sigil_metadata.sigil_fingerprint)
        db.session.add(db_sigil)
        db.session.commit()

        logger.info(f"Created sigil with fingerprint: {sigil_metadata.sigil_fingerprint}")
...
def create_lock():
    try:
        ...
        access_policy = AccessPolicy(
            ...
            required_capabilities=access_policy_data.get('required_capabilities', []),
            attribute_conditions=access_policy_data.get('attribute_conditions', {}),
...
def check_access():
    try:
        ...
        if not sigil_fingerprint or not lock_id:
            return jsonify({"status": "error", "message": "Missing sigil_fingerprint or lock_id"}), 400

        sigil = Sigil.query.filter_by(sigil_fingerprint=sigil_fingerprint).first()
        lock = Lock.query.filter_by(lock_id=lock_id).first()

        if not sigil:
            return jsonify({"status": "error", "message": "Sigil not found"}), 404
        ...
        if access_granted and lock.on_chain_actions:
            for action_data in lock.on_chain_actions:
                try:
                    ...
                    logger.error(f"Error executing on-chain action {action_data.get('function', 'unknown')}: {e}")
                    transaction_receipts.append({
                        "status": "error",
                        "action": action_data.get('function', 'unknown'),
                        "error": str(e)
                    })
...
def list_sigils():
    try:
        ...
        return jsonify({sigil.sigil_fingerprint: sigil.to_dict() for sigil in sigils})
    except Exception as e:
...
def blockchain_status():
    try:
        status = {
            ...
            "block_number": blockchain_service.w3.eth.block_number if blockchain_service.w3.is_connected() else None,
            "service_account": blockchain_service.service_account.address if blockchain_service.service_account else None,
            "contracts": {
                "basic_transparency": blockchain_service.BASIC_TRANSPARENCY_CONTRACT_ADDRESS,
                "care_token": blockchain_service.CARE_TOKEN_CONTRACT_ADDRESS,
                "testimony_nft": blockchain_service.TESTIMONY_NFT_CONTRACT_ADDRESS
            }
        }
